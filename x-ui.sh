...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all nh-v2ray-panel components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/nh-v2ray-panel https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.sh
    chmod +x ${xui_folder}/nh-v2ray-panel.sh
    chmod +x /usr/bin/nh-v2ray-panel
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service nh-v2ray-panel stop
        rc-update del nh-v2ray-panel
        rm /etc/init.d/nh-v2ray-panel -f
    else
        systemctl stop nh-v2ray-panel
        systemctl disable nh-v2ray-panel
        rm ${xui_service}/nh-v2ray-panel.service -f
        systemctl daemon-reload
        systemctl reset-failed
    fi

    rm /etc/nh-v2ray-panel/ -rf
    rm ${xui_folder}/ -rf
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xui_folder}/nh-v2ray-panel setting -username "${config_account}" -password "${config_password}" > /dev/null 2>&1
    else
        ${xui_folder}/nh-v2ray-panel setting -username "${config_account}" -password "${config_password}" -resetTwoFactor=true > /dev/null 2>&1
...
reset_webbasepath() {
    ...
    ${xui_folder}/nh-v2ray-panel setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${xui_folder}/nh-v2ray-panel setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xui_folder}/nh-v2ray-panel setting -show true)
    if [[ $? != 0 ]]; then
    ...
    if [[ -r "$db_env_file" ]] && grep -q '^XUI_DB_TYPE=postgres' "$db_env_file"; then
        ...
        echo -e "${green}Database: SQLite (/etc/nh-v2ray-panel/nh-v2ray-panel.db)${plain}"
    fi

    local existing_webBasePath=$(echo "$info" | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(echo "$info" | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/nh-v2ray-panel setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xui_folder}/nh-v2ray-panel setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $release == "alpine" ]]; then
            rc-service nh-v2ray-panel start
        else
            systemctl start nh-v2ray-panel
        fi
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            LOGI "nh-v2ray-panel Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "nh-v2ray-panel and xray stopped successfully"
        else
...
restart() {
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "nh-v2ray-panel and xray Restarted successfully"
        else
    ...
    if [[ $release == "alpine" ]]; then
        rc-service nh-v2ray-panel restart
    else
...
restart_xray() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service nh-v2ray-panel reload
    else
        systemctl reload nh-v2ray-panel
    fi
    LOGI "xray-core Restart signal sent successfully, Please check the log information to confirm whether xray restarted successfully"
    sleep 2
    show_xray_status
...
status() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service nh-v2ray-panel status
    else
        systemctl status nh-v2ray-panel -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-update add nh-v2ray-panel default
    else
        systemctl enable nh-v2ray-panel
    fi
    if [[ $? == 0 ]]; then
        LOGI "nh-v2ray-panel Set to boot automatically on startup successfully"
    else
        LOGE "nh-v2ray-panel Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "nh-v2ray-panel Autostart Cancelled successfully"
    else
        LOGE "nh-v2ray-panel Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'nh-v2ray-panel[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u nh-v2ray-panel -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-nh-v2ray-panel.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-nh-v2ray-panel.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-nh-v2ray-panel.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-nh-v2ray-panel.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/nh-v2ray-panel -z /usr/bin/nh-v2ray-panel https://github.com/navidhaghpanah/nh-v2ray-panel/raw/main/nh-v2ray-panel.sh
    if [[ $? != 0 ]]; then
        echo ""
...
xui_pid() {
    ps -ef 2> /dev/null | grep -F "${xui_folder}/nh-v2ray-panel" | grep -v grep | awk 'NR==1 {print $1}'
}
...
check_status() {
    if [[ "${running_in_docker}" == "true" ]]; then
        if [[ ! -x "${xui_folder}/nh-v2ray-panel" ]]; then
            return 2
        fi
        if [[ -n "$(xui_pid)" ]]; then
            return 0
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service nh-v2ray-panel status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status nh-v2ray-panel | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'nh-v2ray-panel' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled nh-v2ray-panel)
                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ...
                        if command -v ~/.acme.sh/acme.sh &> /dev/null && ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
                            ~/.acme.sh/acme.sh --installcert -d "${domain}" \
                                ...
                                --reloadcmd "nh-v2ray-panel restart" 2>&1 || true
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xui_folder}/nh-v2ray-panel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart nh-v2ray-panel 2>/dev/null || rc-service nh-v2ray-panel restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="nh-v2ray-panel restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}nh-v2ray-panel restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; nh-v2ray-panel restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; nh-v2ray-panel restart"
                reloadCmd="systemctl reload nginx ; nh-v2ray-panel restart"
                ;;
            2)
                LOGD "It's recommended to put nh-v2ray-panel restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; nh-v2ray-panel restart): " reloadCmd
...
EOF
    ...
    local panel_port
    panel_port=$(${xui_folder}/nh-v2ray-panel setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xui_folder}/nh-v2ray-panel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xui_folder}/nh-v2ray-panel setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xui_folder}/nh-v2ray-panel setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/nh-v2ray-panel setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xui_folder}/nh-v2ray-panel setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${xui_folder}/nh-v2ray-panel setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${xui_folder}/nh-v2ray-panel setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
migrate_to_postgres() {
    if [[ ! -x "${xui_folder}/nh-v2ray-panel" ]]; then
        LOGE "nh-v2ray-panel is not installed."
        return 1
    fi
    echo ""
    ...
    if ! ${xui_folder}/nh-v2ray-panel migrate-db --dsn "$dsn"; then
...
# file using the bundled nh-v2ray-panel binary. With no arguments it dumps the installed
# panel database; an optional second argument overrides the output path.
#   nh-v2ray-panel migrateDB [file.db|file.dump] [output]
migrate_db() {
    local input="$1" output="$2"
    local default_db="/etc/nh-v2ray-panel/nh-v2ray-panel.db"
    local bin="${xui_folder}/nh-v2ray-panel"

    [[ -z "$input" ]] && input="$default_db"

    if [[ ! -x "$bin" ]]; then
        LOGE "nh-v2ray-panel binary not found at ${bin}. Is the panel installed?"
        return 1
    fi

    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "This nh-v2ray-panel build does not support .db <-> .dump conversion yet."
        LOGE "Update the panel first (nh-v2ray-panel update) to a version with 'migrate-db --dump/--restore'."
        return 1
    fi

    if [[ ! -f "$input" ]]; then
        LOGE "Input file not found: ${input}"
        echo -e "Usage: ${green}nh-v2ray-panel migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "Refusing to restore into the live database (${default_db}) while nh-v2ray-panel is running."
            LOGE "Stop the panel first (nh-v2ray-panel stop) or choose a different output path."
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "Output ${output} already exists and will be overwritten. Continue?" "n" || return 0
...
|  ${blue}nh-v2ray-panel restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}nh-v2ray-panel update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}nh-v2ray-panel migrateDB [file]${plain}      - Convert .db <-> .dump (SQLite)   │
