...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload nh-v2ray-panel before expiry.${plain}"
    return 0
}
    ...
    case "$ssl_choice" in
        ...
        4)
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${xui_folder}/nh-v2ray-panel setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
...
config_after_install() {
    local existing_hasDefaultCredential=$(${xui_folder}/nh-v2ray-panel setting -show true | grep -Eo 'hasDefaultCredential: .+' | awk '{print $2}')
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            local db_label="SQLite (/etc/nh-v2ray-panel/nh-v2ray-panel.db)"
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        xui_env_file="/etc/default/nh-v2ray-panel"
                        ;;
                    arch | manjaro | parch | alpine)
                        xui_env_file="/etc/conf.d/nh-v2ray-panel"
                        ;;
                    *)
                        xui_env_file="/etc/sysconfig/nh-v2ray-panel"
                        ;;
                esac
                ...
                while [[ -z "$xui_dsn" ]]; do
                    ...
                    if [[ "$pg_mode" == "2" ]]; then
                        ...
                        local pg_cred_file
                        pg_cred_file=$(mktemp 2> /dev/null) || pg_cred_file=$(mktemp -t nh-v2ray-panel-pg-creds.XXXXXXXX)
...
EOF
            ...
            ${xui_folder}/nh-v2ray-panel setting -username "${config_username}" -password "${config_password}" -port "${config_port}" -webBasePath "${config_webBasePath}"
            ...
            local config_apiToken=$(${xui_folder}/nh-v2ray-panel setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${xui_folder}/nh-v2ray-panel setting -username "${config_username}" -password "${config_password}"
        ...
        # Properly detect empty cert by checking if cert: line exists and has content after it
        existing_cert=$(${xui_folder}/nh-v2ray-panel setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        if [[ -z "$existing_cert" ]]; then
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL Certificate Setup (RECOMMENDED)   ${plain}"
...
install_nh-v2ray-panel() {
    cd ${xui_folder%/nh-v2ray-panel}/

    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/navidhaghpanah/nh-v2ray-panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/navidhaghpanah/nh-v2ray-panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
                echo -e "${red}Failed to fetch nh-v2ray-panel version, it may be due to GitHub API restrictions, please try it later${plain}"
                exit 1
            fi
        fi
        echo -e "Got nh-v2ray-panel latest version: ${tag_version}, beginning the installation..."
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/navidhaghpanah/nh-v2ray-panel/releases/download/${tag_version}/nh-v2ray-panel-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading nh-v2ray-panel failed, please be sure that your server can access GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        url="https://github.com/navidhaghpanah/nh-v2ray-panel/releases/download/${tag_version}/nh-v2ray-panel-linux-$(arch).tar.gz"
        echo -e "Beginning to install nh-v2ray-panel $1"
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download nh-v2ray-panel $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    curl -4fLRo /usr/bin/nh-v2ray-panel-temp https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.sh
    if [[ $? -ne 0 ]]; then
        echo -e "${red}Failed to download nh-v2ray-panel.sh${plain}"
        exit 1
    fi

    # Stop nh-v2ray-panel service and remove old resources
    ...
    tar zxvf nh-v2ray-panel-linux-$(arch).tar.gz
    rm nh-v2ray-panel-linux-$(arch).tar.gz -f

    cd nh-v2ray-panel
    chmod +x nh-v2ray-panel
    chmod +x nh-v2ray-panel.sh
    ...
    chmod +x nh-v2ray-panel bin/xray-linux-$(arch)

    # Update nh-v2ray-panel cli and se set permission
    mv -f /usr/bin/nh-v2ray-panel-temp /usr/bin/nh-v2ray-panel
    chmod +x /usr/bin/nh-v2ray-panel
    mkdir -p /var/log/nh-v2ray-panel
    config_after_install

    # Etckeeper compatibility
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "nh-v2ray-panel/nh-v2ray-panel.db" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "nh-v2ray-panel/nh-v2ray-panel.db" >> "/etc/.gitignore"
                echo -e "${green}Added nh-v2ray-panel.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "nh-v2ray-panel/nh-v2ray-panel.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added nh-v2ray-panel.db for etckeeper${plain}"
        fi
    fi

    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/nh-v2ray-panel https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.rc
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download nh-v2ray-panel.rc${plain}"
            exit 1
        fi
        chmod +x /etc/init.d/nh-v2ray-panel
        rc-update add nh-v2ray-panel
        ...
        if [ -f "nh-v2ray-panel.service" ]; then
            echo -e "${green}Found nh-v2ray-panel.service in extracted files, installing...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "nh-v2ray-panel.service.debian" ]; then
                        echo -e "${green}Found nh-v2ray-panel.service.debian in extracted files, installing...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "nh-v2ray-panel.service.arch" ]; then
                        echo -e "${green}Found nh-v2ray-panel.service.arch in extracted files, installing...${plain}"
                ...
                *)
                    if [ -f "nh-v2ray-panel.service.rhel" ]; then
                        echo -e "${green}Found nh-v2ray-panel.service.rhel in extracted files, installing...${plain}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/nh-v2ray-panel.service https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/nh-v2ray-panel.service https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/nh-v2ray-panel.service https://raw.githubusercontent.com/navidhaghpanah/nh-v2ray-panel/main/nh-v2ray-panel.service.rhel > /dev/null 2>&1
                    ;;
            esac
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}Failed to install nh-v2ray-panel.service file${plain}"
            exit 1
        fi
    fi

    echo -e "${green}nh-v2ray-panel ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
